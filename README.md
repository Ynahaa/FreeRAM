...
从 [Releases](https://github.com/Ynahaa/FreeRAM/releases) 下载 `FreeRAM.exe`，双击运行即可。
...
| 场景 | 命令/操作 |
|------|----------|
| 启动并最小化 | `python main.py --minimized` 或 `FreeRAM.exe -m` |
| 执行一次清理后退出 | `python main.py --once` |
| 查看当前内存状态 | `python main.py --info` |
| 扫描可能的游戏进程 | `python main.py --scan` |
| 指定目标进程 | `python main.py --process Game.exe` |

### 常规使用

1. 启动 FreeRAM，首次运行会在 exe 同目录生成 `config.json`
2. 在设置面板（齿轮图标）中修改目标进程名、阈值等参数
3. 关闭窗口即隐藏到系统托盘，后台持续监控
4. 右键托盘图标 → 退出可彻底关闭
...
或双击运行 `构建.bat` / `build.bat`。

输出的 exe 在 `dist/` 目录中。

## 依赖

- [PySide6](https://pypi.org/project/PySide6/) ≥ 6.5.0 — Qt GUI 框架
- [psutil](https://pypi.org/project/psutil/) ≥ 5.9.0 — 进程和系统监控
- [Pillow](https://pypi.org/project/Pillow/) ≥ 10.0.0 — 图像处理
...
- 普通清理工具无差别释放所有进程内存，**可能导致前台游戏卡顿**
- FreeRAM 通过 Win32 API 精确修剪指定进程，并检测游戏前台状态
- 自身进程设为 IDLE 优先级，运行时几乎不消耗 CPU
